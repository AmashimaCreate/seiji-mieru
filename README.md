...
| `/tools/` | くらしと制度のツール（手取り・年収の壁） | [seiji-tools](https://github.com/AmashimaCreate/seiji-tools)（Vite, `base=/tools/`） |
| `/tottori/` | 地方議会見える化（都道府県議会・政令市・特別区の議員名簿や地域データを整理） | [tottori-mieru](https://github.com/AmashimaCreate/tottori-mieru)（docs 配信構成） |

## ビルド（Cloudflare Pages）
- **Build command**: `bash build.sh`
- **Build output directory**: `dist`

`build.sh` が ① 立法（静的）を `/houan` に、② 外国人マップ（Vite）を `base=/zairyu/` でビルドして `/zairyu` に、③ くらしと制度のツールを `/tools` に、④ 地方議会見える化（docs 配信）を `/tottori` に集約します。
